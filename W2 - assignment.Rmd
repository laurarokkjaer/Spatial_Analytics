...
```{r improved-map}
#I use the tmap library to costumise the map and change the above mentions 
tm_shape(nz_elev)  +
  tm_raster(title = "elev", 
            style = "cont",
            palette = "-RdYlGn") +
  ...
  tm_borders(col = "black", 
             lwd = 1) +
  tm_scale_bar(breaks = c(1, 200, 300),
               text.size = 1) +
  tm_compass(position = c("RIGHT", "top"),
             type = "rose", 
             size = 2) +
  tm_credits(text = "L. Rokkjaer, 10th feb 2022") +
...
* Display the `zion` object and view its structure.
  ...
  it is a tibble with 1 row and 12 columns 
  
  - What type of data does it store? 
  it stores data about a national park called zion national park. 
...
* Display the `srtm` object and view its structure.
  ...
  - What type of data does it store?
  raster data and informations upon the raster data
  ...
  475, 490, 232750  (nrow, ncol, ncell)
...
```{r inspect-data}
#exploring the zion dataset
View(zion)
head(zion)
st_crs(zion)

#exploring the srtm dataset
View(srtm)
head(srtm)
st_crs(srtm)
```
...
```{r}
#reprojecting srtm into zion
srtm2 <- projectRaster(srtm, crs = zion)
plot(srtm2)

#reprojecting zion into srtm
zion2 <- projectRaster(zion, 
                       crs = srtm,
                       res = res(srtm))
