...
data(dem)
plot(dem)
# converting into a raster format
dem <- raster(dem)
```
 Plot in 3D
```{r 3D, eval = FALSE}
...
# remember j3 is red, j2 is green and j1 is blue (it has to come in that RGB order)
...
## better
#using plorRGB for multiband images 
plotRGB(myRGB)
#how to make the raster look better when plotting them - stretch see line 150
plotRGB(myCIR)
```
 ...
 *Any idea what the red color represents in myCIR?*
 The near infered color (the forrest color)
...
```{r ndvi}
# prep imagery
# we need them in raster format, so we can use raster algebra later
n3 <- raster(nov3) # RED
n4 <- raster(nov4) # NIR
...
#reclassify clusters 
#you can then sample the different colors 
plot3D(dem, drape=clusters, col=c("red", "green", "blue", "yellow"))
#assigning different colors to the different clusters 
...
#RECAP:you used the kmeans to cluster and classify an image and checking the values of every band 
...
#besides classifying you can also segment an image 
