...
elevation <- raster("~/Desktop/Spatial analytics/W5 - Raster Data Sources/data/Aster.tif")
...
library(sf)
mounds <- st_read("~/Desktop/Spatial analytics/W5 - Raster Data Sources/data/KAZ_mounds.shp")
st_crs(mounds) == crs(elevation)
crs(elevation)
mounds_newcrs <- st_transform(mounds, 32635)
...
survey <- st_read("~/Desktop/Spatial analytics/W5 - Raster Data Sources/data/KAZ_units.shp")
...
east <- brick("~/Desktop/Spatial analytics/W5 - Raster Data Sources/data/KazE.tif")
...
elevation
prominence <- brick("~/Desktop/Spatial analytics/W5 - Raster Data Sources/data/prominence1500m.tif")
...
```{r prom-mnd-ex}
#plotting the two, like in the above task
plot(prominence)
plot(st_geometry(mounds), add = TRUE)

#using extract to get the values and assigning them to two new columns 
mounds$prom_el_overlay <- raster::extract(elevation_prom_overlay, mounds)
mounds$prominence <- raster::extract(prominence, mounds)

#using crop to put the high-visibility location values into an object
mounds_high_vis <- raster::crop(elevation_prom_overlay, mounds)

#to find the highest 25, we first need to arrange the values descending from top to bottom of the 25 values
mounds_25 <- mounds %>% dplyr::arrange(desc(prominence)) %>% dplyr::slice(1:25)

plot(mounds_high_vis)

#finding the trap code for the 25 mounds using unique
unique(mounds_25$TRAP_Code)

#plotting the mounds, using mapview and comparing their location 
...
sites <- read_sf("~/Desktop/Spatial analytics/W5 - Raster Data Sources/data/KAZ_nuclei.shp")
